/* =========================
   MENU PAGE (Premium Accordion)
   - Day tabs work
   - Nothing auto-opens
   - Category click renders in Focus Mode
   - Food = collapsible sections (accordion)
   - spiritCols ($5 Shots, $10 Drinks) = collapsible sections (accordion)
...
(() => {
  ...
  const emptyStateHTML = `<div class="muted">Select a category above to view items.</div>`;
  ...
  // Accordion: close all sibling <details> in the same container
  function makeAccordion(detailsEl) {
    detailsEl.addEventListener("toggle", () => {
      ...
      // premium: smooth scroll to opened section
  ...
  function renderCategory(scopeKey, catKey) {
    ...
    // ✅ PREMIUM COLLAPSIBLE: spiritCols ($5 Shots / $10 Drinks)
    if (data.type === "spiritCols") {
      const container = document.createElement("div");
      container.className = "accordionStack";
    ...
    // ✅ PREMIUM COLLAPSIBLE: Food sections (accordion)
      ...
      (data.sections || []).forEach((section) => {
        ...
        (section.items || []).forEach(([name, price]) => {
          if (price) {
            ...
            box.appendChild(el(`<div class="itemRow"><span>${name}</span></div>`));
          }
        });

        if (section.note) {
          box.appendChild(el(`<div class="muted" style="margin-top:8px; padding-left:6px;">${section.note}</div>`));
        }
